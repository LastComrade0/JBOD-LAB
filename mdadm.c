...
int mdadm_mount(void) {
  ...
  //Else mount the disk
    ...
    int mount = jbod_operation(disk_op, NULL); //Syscall mount, and will return 0(Success) or 1(Failure)
...
int mdadm_unmount(void) {
  ...
  if (mount_status == 0){
    ...
    int unmount = jbod_operation(disk_op, NULL); //Syscall mount, and will return 0(Success) or 1(Failure)
  ...
  while(num_byte_read < len){
    ...
    if(cache_enabled() == true){
      ...
      if(cache_lookup(disk, block, temp_buf) == -1){
        ...
        int seek_to_disk = jbod_operation(seek_disk_op,NULL);
        if(seek_to_disk == -1){
        ...
        int seek_to_block = jbod_operation(seek_block_op,NULL);
        if(seek_to_block == -1){
        ...
        jbod_operation(read_disk_op, temp_buf); //Read the blocks and copy the value on block to a temporary buffer which is 256 bytes long
  ...
  while(num_byte_write < len){
    ...
    if(cache_enabled() == true){
      ...
      if(cache_lookup(disk, block, temp_buf) == -1){
        ...
        seek_to_disk = jbod_operation(seek_disk_op,NULL);
        if(seek_to_disk == -1){
          printf("Seek to disk failed.");
        }
        ...
        seek_to_block = jbod_operation(seek_block_op,NULL);
        if(seek_to_block == -1){
          printf("Seek to block failed.");
        }
        ...
        jbod_operation(write_disk_op, temp_buf); //Write the updated temp_buf to the block
